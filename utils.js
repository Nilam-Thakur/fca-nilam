    ...
    return function(/** @type {{ body: string; statusCode: string | number; request: { uri: { protocol: string; hostname: string; pathname: string; }; headers: { [x: string]: string; }; formData: any; method: string; }; }} */data) {
        return bluebird.try(function() {
            ...
            if (res.error === 1357001) {
                if (global.Fca.Require.Nilam.AutoLogin && global.Fca.Require.Nilam.CheckPointBypass['956'].Allow) {
...
function getAppState(jar, Encode) {
    ...
    var Language = languageFile.find(i => i.Language == globalThis.Fca.Require.Nilam.Language).Folder.Index;
    var data;
        switch (require(process.cwd() + "/NilamFca.json").EncryptFeature) {
            case true: {
            ...
            case false: {
                ...
                break;
            default: {
                logger.Normal(getText(Language.IsNotABoolean,require(process.cwd() + "/NilamFca.json").EncryptFeature));
                data = appstate;
            } 
        }
