    ...
    Data: new Object({
        ObjNilam: {
            ...
            "AntiStuckAndMemoryLeak": {
                ...
                "LogFile": {
                    "Use": false,
                    "Explain": "Record memory usage logs to fix errors. Default location: Nilam_Database/memory.logs"
                }
            }
        },
        CountTime: function() {
...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/NilamFca.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/NilamFca.json", JSON.stringify(global.Fca.Data.ObjNilam, null, "\t"));
        process.exit(1);
    }

try {
    var Data_Setting = require(process.cwd() + "/NilamFca.json");
}
catch (e) {
    global.Fca.Require.logger.Error('Detect Your NilamFca Settings Invalid!, Carry out default restoration');
    global.Fca.Require.fs.writeFileSync(process.cwd() + "/NilamFca.json", JSON.stringify(global.Fca.Data.ObjNilam, null, "\t"));     
    process.exit(1)
}
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/NilamFca.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                Data_Setting[i] = global.Fca.Data.ObjNilam[i];
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/NilamFca.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
        ...
        for (let i of Object_Fca) {
            const All_Paths = utils.getPaths(global.Fca.Data.ObjNilam[i]);
            const Mission = { Main_Path: i, Data_Path: All_Paths }
            for (let i of Mission.Data_Path) {
                if (Data_Setting[Mission.Main_Path] == undefined) {
                    Data_Setting[Mission.Main_Path] = global.Fca.Data.ObjNilam[Mission.Main_Path];
                    global.Fca.Require.fs.writeFileSync(process.cwd() + "/NilamFca.json", JSON.stringify(Data_Setting, null, "\t"));      
                }
                ...
                if (User_Data_Type == "Number") {
                    ...
                    const Mission_Obj = utils.getData_Path(global.Fca.Data.ObjNilam[Mission.Main_Path], Mission_Path, 0);
    ...
    global.Fca.Require.Nilam = Data_Setting;
}
catch (e) {
    console.log(e);
