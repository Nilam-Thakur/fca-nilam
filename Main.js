...
if (global.Fca.Require.Nilam.Config != 'default') {
    //do ssth
}

const Language = global.Fca.Require.languageFile.find((/** @type {{ Language: string; }} */i) => i.Language == global.Fca.Require.Nilam.Language).Folder.Index;
...
function buildAPI(globalOptions, html, jar, bypass_region) {
    ...
    if (maybeUser.length === 0 && maybeTiktik.length === 0) {
        if (global.Fca.Require.Nilam.AutoLogin) {
            return global.Fca.Require.logger.Warning(global.Fca.Require.Language.Index.AutoLogin, function() {
                global.Fca.Action('AutoLogin')
            });
        }
        else if (!global.Fca.Require.Nilam.AutoLogin) {
        ...
        fs.readdirSync(__dirname + "/src").filter((/** @type {string} */File) => File.endsWith(".js") && !File.includes('Dev_')).map((/** @type {string} */File) => { 
            if (File == 'getThreadInfo.js' && global.Fca.Require.Nilam.AntiGetInfo.AntiGetThreadInfo != true || File == 'getUserInfo.js'  && global.Fca.Require.Nilam.AntiGetInfo.AntiGetUserInfo != true) api[File.split('.').slice(0, -1).join('.')] = require('./src/' + (File.includes('getThreadInfo') ? 'getThreadMain.js' : 'getUserInfoMain.js'))(defaultFuncs, api, ctx)
                    ...
                    return utils
                        ...
                        .then(function(res) {
                            ...
                            if (html.indexOf("checkpoint/?next") > -1) {
                                ...
                                switch (global.Fca.Require.Nilam.Login2Fa) {
                                    case true: {
                                        ...
                                        const Code = global.Fca.Require.Nilam.AuthString.includes('|') == false ? Otp_code(global.Fca.Require.Nilam.AuthString.includes(" ") ? global.Fca.Require.Nilam.AuthString.replace(RegExp(" ", 'g'), "") : global.Fca.Require.Nilam.AuthString) :  question(Language.EnterSecurityCode); 
                                            try {
...
try {
    if (appState) {
        logger.Normal(Language.OnProcess);
            ...
            try {
                switch (global.Fca.Require.Nilam.EncryptFeature) {
                    case true: {
                    ...
                    case false: {
                        ...
                        break;
                    default: {
                        logger.Warning(getText(Language.IsNotABoolean,global.Fca.Require.Nilam.EncryptFeature))
                        process.exit(0);
                    }
                }
            }
    ...
    try {
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && global.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (global.Fca.Require.Nilam.ResetDataLogin) {
                        global.Fca.Require.Nilam.ResetDataLogin = false;
                        global.Fca.Require.fs.writeFileSync(process.cwd() + '/NilamFca.json', JSON.stringify(global.Fca.Require.Nilam, null, 4));
                    }
        ...
        if (global.Fca.Require.FastConfig.Websocket_Extension.Status) {
            ...
            if (!UserName || !PassWord || global.Fca.Require.FastConfig.Websocket_Extension.ResetData) {
                ...
                if (!UserName || !PassWord) {
                    ...
                    global.Fca.Require.fs.writeFileSync(process.cwd() + "/NilamFca.json", JSON.stringify(global.Fca.Require.FastConfig, null, "\t"));
                }
                else {
                    try {
...
**/
        switch (global.Fca.Require.Nilam.AutoLogin) {
            case true: {
                if (global.Fca.Require.Nilam.ResetDataLogin) return setUserNameAndPassWord();
                else {
                    try {
