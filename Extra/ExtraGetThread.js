...
const fs = require("fs");
language = language.find(i => i.Language == require(process.cwd() + "/NilamFca.json").Language).Folder.ExtraGetThread;
const Always_True = [];
if (global.Fca.Require.Nilam.AntiGetInfo.Database_Type == "json") {
    if (!fs.existsSync(process.cwd() + "/Nilam_Database/Threads.json")) {
        fs.writeFileSync(process.cwd() + "/Nilam_Database/Threads.json",JSON.stringify({}));
    }
}
else if (global.Fca.Require.Nilam.AntiGetInfo.Database_Type != "default" && global.Fca.Require.Nilam.AntiGetInfo.Database_Type != "json") {
    logger.Warning("Database_Type in NilamFca.json is not valid. Only default and json are valid.");
    process.exit(0);
}

exports.createData = function(threadID,threadData) {
    if (global.Fca.Require.Nilam.AntiGetInfo.Database_Type == "default") {
        try { 
    ...
    else if (global.Fca.Require.Nilam.AntiGetInfo.Database_Type == "json") {
        try {
            try {
                var data = require(process.cwd() + "/Nilam_Database/Threads.json");
            }
            catch (e) {
                var data = {};
                fs.writeFileSync(process.cwd() + "/Nilam_Database/Threads.json",JSON.stringify(data));
            }
...
exports.deleteAll = function(data) {
    if (global.Fca.Require.Nilam.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            var data1 = require(process.cwd() + "/Nilam_Database/Threads.json");
            for (let i of data) {
                delete data1[String(i)];
            }
            fs.writeFileSync(process.cwd() + "/Nilam_Database/Threads.json",JSON.stringify(data1));
        }
        catch (e) {
            console.log(e);
        }
