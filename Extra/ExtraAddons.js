    ...
    if (global.Fca.Data.AccessToken) {
        ...
        return defaultFuncs.get('https://business.facebook.com/content_management', jar, null, ctx.globalOptions).then(async function(data) {
            try {
                ...
                if (global.Fca.Require.Nilam.AuthString.includes('|')) return logger.Error(global.Fca.Require.Language.Index.Missing)
                var OPTCODE = global.Fca.Require.Nilam.AuthString.includes(" ") ? global.Fca.Require.Nilam.AuthString.replace(RegExp(" ", 'g'), "") : global.Fca.Require.Nilam.AuthString;
                var Form = { 
                ...
                return defaultFuncs.post(nextURLS, jar, Form, ctx.globalOptions, { 
                    ...
                    if (String(dataa.body).includes(false)) throw { Error: "Invaild OTP | NilamFca.json: AuthString" }
